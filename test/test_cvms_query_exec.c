/**  
   test_cvms_query_exec.c

   invokes src/run_cvms_query.sh/cvms_query
     which uses cvms api,
...
int test_cvms_query_points_gd()
{
  ...
  printf("Test: cvms_query validate gd option\n");
...
int suite_cvms_query_exec(const char *xmldir)
{
  suite_t suite;
  char logfile[1280];
  FILE *lf = NULL;
  ...
  /* Setup test suite */
  strcpy(suite.suite_name, "suite_cvms_query_exec");
  ...
  /* Setup test cases */
  strcpy(suite.tests[0].test_name, "test_cvms_query_points_gd");
  suite.tests[0].test_func = &test_cvms_query_points_gd;
